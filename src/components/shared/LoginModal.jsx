...
const LoginModal = ({ open, onOpenChange, onLogin }) => {
  ...
  const openForgotPassword = () => {
    const prefillEmail =
      formData.email?.trim() || resetData.email?.trim() || "";
    setView("forgot");
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      ...
      setError(
        err.message || "Não foi possível enviar o OTP. Tente novamente.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmOtp = async (e) => {
    ...
    try {
      ...
      if (res?.success) {
        setInfo(
          "OTP verificado com sucesso. Agora defina uma nova palavra-passe.",
        );
        setResetStep("reset");
      } else {
      ...
      setError(
        err.message || "Não foi possível confirmar o OTP. Tente novamente.",
      );
    } finally {
      setLoading(false);
    }
